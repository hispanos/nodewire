...
class NodeWireRuntime {
        ...
        try {
            ...
            this.ws.onmessage = (event) => {
                try {
                    ...
                    // Manejar actualizaciones reactivas (stateUpdate)
                    if (result.type === 'stateUpdate') {
                        this.handleStateUpdate(result);
                    } else {
                        // Mensaje normal de respuesta
    ...
    /**
     * Maneja actualizaciones reactivas de propiedades que empiezan con $
     */
    handleStateUpdate(result) {
        const { componentId, updates, html, newState } = result;
        
        console.log('[NodeWire] Actualización reactiva recibida:', { componentId, updates });
        
        // Actualizar solo los elementos afectados por las propiedades reactivas
        if (updates && Object.keys(updates).length > 0 && html) {
            this.updateElements(componentId, html, newState, updates);
        }
    }
            ...
            for (const prop in updates) {
                ...
                currentElementsForProp.forEach((currentEl, index) => {
                    ...
                    if (newEl) {
                        ...
                        // Actualizar atributos (especialmente disabled)
                        Array.from(newEl.attributes).forEach(attr => {
                            if (attr.name === 'disabled') {
                                // Si el nuevo elemento tiene disabled, agregarlo
                                currentEl.setAttribute('disabled', '');
                            } else if (attr.name !== 'data-nodewire-id' && attr.name !== 'data-nodewire-prop' && attr.name !== 'data-nodewire-component') {
                                // Actualizar otros atributos (excepto los de NodeWire)
                                currentEl.setAttribute(attr.name, attr.value);
                            }
                        });
                        
                        // Si el nuevo elemento NO tiene disabled pero el actual sí, removerlo
                        if (!newEl.hasAttribute('disabled') && currentEl.hasAttribute('disabled')) {
